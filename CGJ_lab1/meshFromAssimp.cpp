...
bool LoadGLTexturesTUs(const aiScene* scene, const char* model_dir)  // Create OGL textures objects and maps them to texture units.  
{
...
vector<struct MyMesh> createMeshFromAssimp(const aiScene* sc, const char* model_dir) {
	...
	LoadGLTexturesTUs(sc, model_dir); //it creates the unordered map which maps image filenames to texture units TU
